#!/usr/bin/env python3
"""
route_add_script_multiple1.py

Connects to multiple Cisco IOS routers over SSH (one after another, in a
loop), adds the same static route to each, verifies it, saves the
configuration, and displays confirmation output.

Router IP addresses are no longer typed in manually. Instead, the user is
...
def get_inputs():
    ...
    route configuration details. These credentials and route settings
    are reused for every device read from the CSV.
    """
    ...
    # --- New static route details (also shared across all devices) ---
...
def process_device(ip_address, data, route_command):
    """
    Connect to a single router and run the full add-route workflow:
    add the route, verify it, save the config if successful, and log
    the verification output. Returns True on overall success, False on
    any failure, so the caller's loop can keep going to the next device.
    """
    ...
    try:
        ...
        # Add the new route to the running config.
        ...
        # Display all static routes currently in the running config
        show_output = conn.send_command("show run | include ip route")
        print("\n--- show run | include ip route ---")
        print(show_output)

        # Confirm the exact route line is present before saving.
        ...
        # Show just the running-config line(s) matching the new
    ...
    # exiting the whole script) so the loop can move on to the next IP.
        ...
        print(f"Authentication failed for {ip_address}. Check username/password/enable password.")
        return False
...
def main():
    ...
    # This is the same command applied to every device in the loop.
    ...
    # full add-route workflow against each device in turn.
    for ip_address in data["ip_addresses"]:
        result = process_device(ip_address, data, route_command)
        if result:
